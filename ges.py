...
class GestureRecognizer:
        ...
        if results.multi_hand_landmarks:
            for idx, hand_landmarks in enumerate(results.multi_hand_landmarks):
                ...
                if hand_local:
                    ...
                    if gesture_str:
                        detected_gestures.append(gesture_str)
                        if self.display:
    ...
    def stop(self):
        ...
        self.running = False
        if self.display:
            cv2.destroyWindow("Gesture Recognition")

if __name__ == '__main__':
    recognizer = GestureRecognizer()
    recognizer.start(display=True)
    
    try:
        while True:
            gestures = recognizer.get_gesture()
            if gestures:
                print("检测到手势:", gestures)
